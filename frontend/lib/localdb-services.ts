...
let syncHandlers: any[] = [];

export const initializeSync = (userId: string | null) => {
  // 1. Cancel existing syncs if any (e.g. user switch or logout)
  syncHandlers.forEach(handler => handler.cancel());
  syncHandlers = [];

  if (!userId) {
    console.log('Guest mode: Sync disabled.');
    return;
  }

  const couchUrl = process.env.NEXT_PUBLIC_COUCHDB_URL;
  if (!couchUrl) {
    console.warn('Sync skipped: NEXT_PUBLIC_COUCHDB_URL not set.');
    return;
  }
  ...
  // We use a pattern: remote_db_name = user_{userId}_{dbName}
  // This ensures data separation per user on the remote side.
  
  // Define the databases we want to sync
  const dbsToSync = [
  ...
  dbsToSync.forEach(({ name, db }) => {
    const remoteName = `user_${userId}_${name}`;
    // Construct remote URL with credentials if they were embedded in env, 
    // but usually treating the URL as base. 
    // If NEXT_PUBLIC_COUCHDB_URL includes credentials (http://user:pass@host), PouchDB handles it.
    // We need to append the ref to the specific DB.
    
    // Ensure trailing slash handling
    const baseUrl = couchUrl.endsWith('/') ? couchUrl : `${couchUrl}/`;
    const remoteDB = `${baseUrl}${remoteName}`;

    console.log(`Starting sync for ${name} -> ${remoteName}`);
    ...
    const sync = db.sync(remoteDB, {
      ...
      // handle change
      console.log(`[Sync ${name}] Change:`, info);
    }).on('paused', (err) => {
      // replication paused (e.g. replication up to date, user went offline)
      console.log(`[Sync ${name}] Paused`, err);
    }).on('active', () => {
      // replicate resumed (e.g. new changes replicating, came back online)
      console.log(`[Sync ${name}] Active`);
    }).on('denied', (err) => {
      // a document failed to replicate (e.g. due to permissions)
      console.error(`[Sync ${name}] Denied:`, err);
    }).on('complete', (info) => {
      // handle complete
      console.log(`[Sync ${name}] Complete:`, info);
    }).on('error', (err) => {
      // handle error
